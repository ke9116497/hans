...
{
    printf(
        "Hans - IP over ICMP version 0.4.4_modified\n\n"
        "RUN AS SERVER\n"
        "  hans -s network [-fvr] [-p password] [-u unprivileged_user] [-d tun_device] [-b bind_address] [-m reference_mtu] [-a ip]\n\n"
        "RUN AS CLIENT\n"
        "  hans -c server  [-fv]  [-p password] [-u unprivileged_user] [-d tun_device] [-b bind_address] [-m reference_mtu] [-w polls]\n\n"
        "ARGUMENTS\n"
        ...
        "  -b bind_addr  Bind to the given local address for sending ICMP.\n"
...
int main(int argc, char *argv[])
{
    ...
    int c;
    while ((c = getopt(argc, argv, "fru:d:p:s:c:m:w:qivb:a:")) != -1)
    {
        switch(c) {
            case 'f':
                foreground = true;
    ...
    try
    {
        if (isServer)
        {
            worker = new Server(mtu, device, password, network, answerPing, uid, gid, 5000, bindIp);
        }
        else
        {
            ...
            worker = new Client(mtu, device, ntohl(serverIp), maxPolls, password, uid, gid, changeEchoId, changeEchoSeq, clientIp, bindIp);
        }
